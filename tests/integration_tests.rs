...
async fn test_write_config_flag() {
    ...
    let parsed_config: toml::Table =
        toml::from_str(&config_content).expect("Generated config is not valid TOML");

    // Verify the AWS section
    let aws_section = parsed_config
        .get("aws")
    ...
    assert_eq!(
        aws_section
            .get("region")
            .and_then(|v| v.as_str())
            .expect("Region should be a string"),
        "us-west-2"
    );
    ...
    assert_eq!(
        aws_section
            .get("access_key_id")
            .and_then(|v| v.as_str())
            .expect("Access key should be a string"),
        "test_access_key"
    );
    ...
    assert_eq!(
        aws_section
            ...
            .and_then(|v| v.as_str())
            .expect("Secret key should be a string"),
        "test_secret"
    );

    // Verify the records section
    let records = parsed_config
        .get("records")
    ...
    assert_eq!(
        first_record
            .get("hosted_zone_id")
            .and_then(|v| v.as_str())
            .expect("Hosted zone ID should be a string"),
        "Z123456789"
    );
    ...
    assert_eq!(
        first_record
            ...
            .expect("TTL should be an integer"),
        300
    );
...
async fn test_write_config_with_other_flags_fails() {
    ...
    assert!(
        !output.status.success(),
        "Command should fail when using --write-config with --once"
    );

    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("--write-config cannot be used with other flags"),
        "Should show error message about conflicting flags. Actual stderr: {}",
        stderr
    );
    ...
    assert!(
        !output.status.success(),
        "Command should fail when using --write-config with --no-aws"
    );
...
"#;
    ...
    assert!(
        stdout.contains("Running in dry-run mode (--no-aws)"),
        "Should indicate dry-run mode. Actual stdout: {}",
        stdout
    );
    ...
    assert!(
        ...
        "Should contain dry-run log messages. Actual stdout: {}",
        stdout
    );

    // Verify that it shows what would be done
    assert!(
        stdout.contains("Would update DNS record"),
        "Should show what DNS update would be performed. Actual stdout: {}",
        stdout
    );

    // Verify that AWS API calls are mentioned as simulated
    assert!(
        stdout.contains("AWS Route53 API call would be made"),
        "Should mention simulated AWS API calls. Actual stdout: {}",
        stdout
    );
}
